...
export class TenantGuard implements CanActivate {
  ...
  canActivate(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): boolean {
    if (this.authService.loggedUser.value) {
      if (this.authService.loggedUser.value.role === 'ROLE_TENANT') {
        return true;
      } else if (this.authService.loggedUser.value.role === 'ROLE_LANDLORD') {
        ...
        try {
          ...
          if (param1 === 'ad') {
            this.router.navigate([`/landlord-dashboard/ad/${param2}`]);
            return false;
          }
        } catch (error) {}
